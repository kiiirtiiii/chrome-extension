...
const scrapeDataAndGenerateSummary = async (req, res, next) => {
  try {
    ...
    const prompt = req.query.regenerate ? `${prompts.REGENERATE_SUMMARY} in ${req.query.response_token} characters?\n\n${processedText}`:`${prompts.GET_SUMMARY} in ${req.query.response_token} characters\n\n${processedText}?`;
    ...
    const response = await client.completions.create({
      ...
      prompt,
      max_tokens: Math.floor(+req.query.response_token / 4),  // length of the summary
